...
def build_state(observation):
    ...
    state = [discretize_value(obs, state_bins[i]) for i, obs in enumerate(observation)]
    state = int("".join(map(lambda feature: str(int(feature)), state)))
    return state
...
if __name__ == '__main__':
    ...
    print('Reward of Test agent = %.3f' % play_and_train(env, agent, visualize=False))
