...
os.environ["PATH"] +=os.pathsep + 'C:/Program Files (x86)/graphviz/graphviz-2.38/release/bin/'
...
def get_action_value(mdp, state_values, state, action, gamma):
    ...
    for next_state, prob in mdp.get_next_states(state, action).items():
        ir = mdp.get_reward(state, action, next_state)
        q += prob * (ir + state_values[next_state] * gamma)
    return q
...
def get_new_state_value(mdp, state_values, state, gamma):
    ...
    values = (get_action_value(mdp, state_values, state, action, gamma) for \
                action in mdp.get_possible_actions(state))
