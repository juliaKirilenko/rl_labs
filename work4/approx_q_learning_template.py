...
def define_network(state_dim, n_actions):
    ...
    network.add_module('module1', nn.Linear(state_dim[0], 42))
    network.add_module('module1_relu', nn.ReLU())
    network.add_module('module2', nn.Linear(42, 102))
    network.add_module('module2_relu', nn.ReLU())
    network.add_module('module3', nn.Linear(102, n_actions))
    return network
...
def get_action(state, epsilon=0):
    ...
    """
    state = Variable(torch.FloatTensor(state)) #, dtype=torch.DoubleTensor))
